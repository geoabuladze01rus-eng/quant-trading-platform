...
function AuditBrowser() {
  ...
  return <details><summary>Подробный журнал решений</summary>
    ...
    {error && <p role="alert">{error}</p>}{page ? <><p>{page.total} событий · начиная с записи {page.offset}</p><Records records={page.items} /><button disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - 50))}>Previous</button><button disabled={offset + page.items.length >= page.total} onClick={() => setOffset(offset + 50)}>Next</button></> : <p>Загружаем журнал…</p>}
  </details>;
}
